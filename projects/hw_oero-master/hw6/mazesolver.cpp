...
void MazeSolver::solveByDFSRecursive()
{
    ...
    DFSRecursiveHelper(r, c, numExplored, parent, vt, st);  
...
void MazeSolver::DFSRecursiveHelper(int r, int c, int numExplored, vector<Direction>& parent, VisitedTracker& vt, std::stack<Direction>& st)
{
        ...
        */
        ...
        if ( maze->canTravel(RIGHT, r, c) && ! vt.isVisited(r, c+1) )
        {
            ...
            vt.setVisited(r, c+1);
            DFSRecursiveHelper(r, c+1, numExplored+1, parent, vt, st);
            if(stuff == true)
            {
                st.push( parent[squareNumber(r, c+1)] );
                return;
            }
        ...
        if ( maze->canTravel(LEFT, r, c) && ! vt.isVisited(r,c-1) )
        {
            ...
            //how is it going to know when to stop
        ...
        if ( maze->canTravel(DOWN, r, c) && ! vt.isVisited(r+1,c) )
        {
            ...
            DFSRecursiveHelper(r+1, c, numExplored+1, parent, vt, st);
            if(stuff == true)
            {
                st.push( parent[squareNumber(r+1, c)] );
                return;
            }
